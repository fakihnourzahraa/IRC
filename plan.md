...
- Operator vs. regular-user permission checks for all of the above

## Shared 

- Agree on the `Client` and `Channel` class interfaces before implementing them separately
- Agree on the full set of numeric replies both sides will need, so the format is consistent
- Makefile skeleton
...
- Output buffer, if needed, for queued outgoing data
